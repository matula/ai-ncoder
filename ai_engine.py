...
- For volume adjustment: use `-af volume=X` where X is a float multiplier. 50% louder = `volume=1.5`, twice as loud = `volume=2.0`, 50% softer/quieter = `volume=0.5`. When adjusting volume on a video file, copy the video stream with `-c:v copy`.
...
Input: Format: MP3 (MPEG audio layer 3), Duration: 00:03:30, Audio: mp3 44100Hz 2ch
File: <INPUT_1>.mp3
User: "make it 50% louder"
Output: {"command": "ffmpeg -i <INPUT_1>.mp3 -map a -af volume=1.5 <OUTPUT_1>.mp3", "is_ambiguous": false, "clarification_question": ""}

Input: Format: MPEG-4, Duration: 00:02:00, Video: h264 1920x1080, Audio: aac 48000Hz 2ch
File: <INPUT_1>.mp4
User: "make it quieter"
Output: {"command": "ffmpeg -i <INPUT_1>.mp4 -af volume=0.5 -c:v copy <OUTPUT_1>.mp4", "is_ambiguous": false, "clarification_question": ""}
